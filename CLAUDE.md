...
.github/workflows/      CI on every PR; schema drift quarterly
...
`rake schema:drift` is deliberately *not* part of that default. It needs a clone
of `schemaorg/schemaorg`, which a contributor has no reason to have; the task
prints the exact clone command when it cannot find one. Run quarterly by
