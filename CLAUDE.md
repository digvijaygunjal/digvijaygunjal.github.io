...
The field list below was checked against **release 30.0**. When a newer release
lands, re-check three things: that no property emitted here has gained a
`schema:supersededBy`, that the `NutritionInformation` key list is unchanged,
and that every `suitableForDiet` value is still a member of `RestrictedDiet`.
...
Rakefile                 the entry point: `bundle exec rake` runs the tiers
test/support/            shared loaders; the vocabularies and the contract
test/tier1/              data and front matter, straight from the sources
```
